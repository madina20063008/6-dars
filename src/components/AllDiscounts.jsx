...
const AllDiscounts = ({ token }) => {
  ...
  return (
    <div className="bg-green-200 mt-[20px] pt-[10px] p-[10px]">
      <h2 className="text-green-700 font-semibold text-[30px] text-center">{discount.title}</h2>
      <p className="text-center font-bold mt-[10px] text-[20px]"> Up To: {discount.discoount_up_to}%</p>
      <img src={discount.poster_image_url} alt={discount.title} style={{ width: "300px" }} />
    </div>
  );
};
